...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id            = $_POST['id']             ?? '';
    $title         = $_POST['title']          ?? '';
    $applicantUnit = trim($_POST['applicantUnit'] ?? '');   // ← 申請單位
    ...
    foreach ($data as &$item) {
        if ($item['id'] === $id) {
            $oldTitle        = $item['title'];
            $category        = $item['status'] ?? '';   // ← 從現有資料讀取公告大項
            $item['title']   = $title;
            ...
            // 判斷是一般編輯還是有上傳新檔（覆蓋公告）
            $hasNewFile = (isset($_FILES['img']) && $_FILES['img']['error'] === UPLOAD_ERR_OK)
                       || (isset($_FILES['pdf']) && $_FILES['pdf']['error'] === UPLOAD_ERR_OK);
            $auditAction = $hasNewFile ? '覆蓋公告' : '編輯公告';

            // 組合 audit detail：標題【類別：xxx】【申請：xxx】
            $logDetail = $oldTitle !== $title ? "標題：{$oldTitle} → {$title}" : "標題：{$title}";
            $logDetail .= "【類別：{$category}】";
            if ($applicantUnit !== '') {
                $logDetail .= "【申請：{$applicantUnit}】";
            }
            break;
        }
    }
    ...
    if ($success && file_put_contents($dataFile, json_encode($data, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT))) {
        writeAuditLog($auditAction ?? '編輯公告', $logDetail ?? '');
        echo json_encode([
            'success' => true,
